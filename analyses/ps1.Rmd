...
```
We can't go forward with the analysis because:  
* R is treating our quantitative variables as factors (except for `Input.price3`). 
* There are 90 observations but only 87 unique worker IDs (Factor w/ 87 levels) which means some people did the task more than once
```
...
fix_vars <- c('Answer.dog_cost', 'Answer.plasma_cost', 'Answer.sushi_cost', 'Input.price1', 'Input.price2')
for (fv in fix_vars) d[[fv]] <- as.numeric(d[[fv]])

# only retain first occurrence of a given worker
...
```{r select}
d.tidy <- select(d, starts_with('Answer'), starts_with('Input'), WorkerId)
```
...
```{r rename}
d.tidy <- d.tidy %>% rename(worker        = WorkerId,
...
```{r gather}
d.tidy <- d.tidy %>%
          gather(cell, price, dog_cost:sushi_cost, dog_anchor:sushi_anchor, na.rm = TRUE) %>%
          separate(cell, into = c('item', 'type'), sep = '_')
# used blog.rstudio.org/2014/07/22/introducing-tidyr/
...
```{r spread}
d.wide <- d.tidy %>%
          transform(cell = paste(item, type, sep = '_'), item = NULL, type = NULL) %>%
          spread(cell, price)

head(d.wide) # back to original wide
```
...
```{r}
ggplot(subset(d.tidy, type == 'cost'), aes(x = price)) + geom_histogram()
```

```
We can see three distinct modes to the distribution, corresponding to a mixture of the three item distributions. Each item distribution looks normal-ish, although it is hard to tell for the cheapest item because of the scale.
```
...
```{r}
summarise(subset(d.tidy, type == 'cost'), mean(price))
```
...
pcts %>%  
group_by(condition, item) %>%
summarise(mean(pct_change, na.rm = TRUE)) #alternatively could specify na.rm = TRUE in call to gather()
```
...
# edited plotting code -- maybe i misunderstood previously and pcts was supposed to be the summarized data frame rather than subject-wise? but this should be equivalent.
```
...
# edited plotting code -- maybe i misunderstood previously and z.scores was supposed to be the summarized data frame rather than subject-wise? but this should be equivalent. (same as above)
```
